...
function Sleep() {
    ...
    const [sleepData, setSleepData] = useState([]); // changed for firebase
    const [loading, setLoading] = useState(true); //changed for firebase

    const monthStart = startOfMonth(currentDate);
    const monthEnd = endOfMonth(currentDate);
    const daysInMonth = eachDayOfInterval({ start: monthStart, end: monthEnd });

    // Load sleep data from Firestore on mount
    useEffect(() => {
        const loadData = async () => {
            const data = await storage.getSleepData();
            setSleepData(data);
            setLoading(false);
        };
        loadData();
    ...
    const handleSaveSleep = async (sleepEntry) => {
        console.log('Saving entry:', sleepEntry);
        const existing = sleepData.find(e => e.date === sleepEntry.date);
        try {
            if (existing) {
                await storage.updateSleepEntry(existing.id, sleepEntry);
            } else {
                await storage.addSleepEntry(sleepEntry);
            }
            console.log('Save successful!');  // ← add this
        } catch (error) {
            console.error('Save failed:', error);  // ← and this
        }
        // Reload fresh data from Firestore
        const updated = await storage.getSleepData();
    ...
    const handleDeleteEntry = async (dateKey) => {
        const entry = sleepData.find(e => e.date === dateKey);
        if (entry) {
            await storage.deleteSleepEntry(entry.id);
            const updated = await storage.getSleepData();
            setSleepData(updated);
        }
        setIsModalOpen(false);
